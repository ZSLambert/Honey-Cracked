...
void readAndConvertAccel() {
  ...
  accelXToG = accelerometer.convertToG(LIS331::HIGH_RANGE, accelX);
  accelYToG = accelerometer.convertToG(LIS331::HIGH_RANGE, accelY);
  accelZToG = accelerometer.convertToG(LIS331::HIGH_RANGE, accelZ);
}
...
void arcadeState() {
  ...
  leftDrivePower = pwmCheck((rx_forwardsBackwards + rx_leftRight) - k_pwmCenter);
  rightDrivePower = pwmCheck((rx_forwardsBackwards - rx_leftRight) + k_pwmCenter);
